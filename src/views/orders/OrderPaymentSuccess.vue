  ...
  <div class="payment-success">
    <div class="success-card">
      ...
      <div v-if="orderData" class="order-details">
        ...
        <div class="train-info">
          ...
          <div class="train-route">
            ...
            <div class="route-info">
              <div class="seat-type">
                {{ orderData.seatType || "Standard Seat" }}
              </div>
              <div class="duration">
                {{
                  orderData.runTime ||
                  calculateDuration(orderData.fromTime, orderData.toTime)
                }}
              </div>
            </div>
        ...
        <!-- 乘客信息 -->
        <div
          class="passengers-info"
          v-if="orderData.passengers && orderData.passengers.length > 0"
        >
          ...
          <div class="info-grid">
            <div class="info-item">
              ...
              <div>
                {{
                  orderData.payment
                    ? capitalizeFirstLetter(orderData.payment.paymentMethod)
                    : "PayPal"
                }}
              </div>
            </div>
            <div class="info-item">
              ...
              <div>
                {{
                  orderData.payment
                    ? formatDate(new Date(orderData.payment.updatedAt))
                    : formatDate(new Date())
                }}
              </div>
            </div>
            ...
            <div class="info-item">
              ...
              <div class="status success">
                {{
                  getPaymentStatus(
                    orderData.payment ? orderData.payment.paymentStatus : 1
                  )
                }}
              </div>
            </div>
            <div
              class="info-item"
              v-if="orderData.payment && orderData.payment.paymentId"
            >
        ...
        <div class="contact-info" v-if="orderData.contact">
          ...
          <div class="info-grid">
            <div class="info-item">
              <label>Name</label>
              <div>
                {{ orderData.contact.title }} {{ orderData.contact.name }}
              </div>
            </div>
            <div class="info-item">
              <label>Email</label>
        ...
        <div class="steps">
          <div class="step">
            ...
            <div class="step-content">
              ...
              <p>
                We've sent your e-tickets and payment confirmation to your email
          ...
          <div class="step">
            ...
            <div class="step-content">
              ...
              <p>
                Download and save your e-tickets. You'll need to show them
                during your journey.
              </p>
            </div>
          </div>
          ...
          <div class="step">
            ...
            <div class="step-content">
              ...
              <p>
                Make sure to arrive at the station at least 30 minutes before
                departure. Don't forget to bring valid ID for all passengers.
              </p>
            </div>
          </div>
        </div>
      ...
      <div class="contact-support">
        <p>
          Need help?
          <el-link type="primary" href="#" @click.prevent="contactSupport"
...
const totalAmount = computed(() => {
  const seatFee =
    currencyStore.convertPrice(orderData.value.seatFee) *
    orderData.value.passengers.length;
  let seatPrice = 0;
...
function calculateDuration(departTime, arriveTime) {
  ...
  // 处理跨天的情况
  if (
    arriveHours < departHours ||
    (arriveHours === departHours && arriveMinutes < departMinutes)
  ) {
    arriveHours += 24;
  }

  const totalMinutes =
    (arriveHours - departHours) * 60 + (arriveMinutes - departMinutes);
...
const viewBookings = () => {
  router.push(`/orders?bookingId=${orderId.value}`);
};
...
const downloadOrderDetails = () => {
  ...
  if (orderData.value.passengers && orderData.value.passengers.length > 0) {
    ...
    orderData.value.passengers.forEach((passenger, index) => {
      passengersInfo += `${index + 1}. ${
        passenger.passportName
      } (${getPassengerType(passenger.passengerType)}) - Passport: ${
...
From: ${formatStation(orderData.value.fromStation, allCities.value)}
To: ${formatStation(orderData.value.toStation, allCities.value)}
Departure: ${orderData.value.fromTime || "N/A"}
Arrival: ${orderData.value.toTime || "N/A"}
Seat Type: ${orderData.value.seatType || "Standard Seat"}
Amount Paid: ${currencyStore.currencySymbol} ${totalAmount.value}
${passengersInfo}
Payment Method: ${
    orderData.value.payment
      ? capitalizeFirstLetter(orderData.value.payment.paymentMethod)
      : "PayPal"
  }
Payment ID: ${
    orderData.value.payment ? orderData.value.payment.paymentId : "N/A"
  }
Payment Status: ${getPaymentStatus(
    orderData.value.payment ? orderData.value.payment.paymentStatus : 1
  )}
